...
Common issues and their solutions when using Plan-Build-Run.
...
**Solutions**:
1. Use `/pbr:pause` to save state, then start a fresh session with `/pbr:resume`
...
**Prevention**: Plan-Build-Run's `track-context-budget.js` hook warns after 20 file reads. The `suggest-compact.js` hook warns when writes push context toward limits. Heed these warnings early.

## Plan Checker Rejection Loops

**Symptom**: `/pbr:plan` keeps failing the plan-checker step and cycling back to revisions.

**Solutions**:
1. Run `/pbr:discuss N` first to clarify requirements before planning
2. Use `/pbr:plan N --skip-research` if research is adding noise
...
**Symptom**: `/pbr:review N` reports must-haves as "not found" even though you implemented them.

**Solutions**:
1. Check must-have wording vs actual code — the verifier searches for exact patterns
2. Use `/pbr:review N --auto-fix` to auto-diagnose and create gap-closure plans
3. Run `/pbr:plan N --gaps` to create targeted fix plans for specific failures
...
**Symptom**: `/pbr:status` shows wrong phase, progress bar is stuck, or commands fail with "STATE.md not found".

**Solutions**:
1. Run `/pbr:health` — it diagnoses and can auto-fix common state corruption
...
**Solutions**:
1. Run `/pbr:resume` — it reads `.planning/STATE.md` and suggests the next action
...
4. Re-run `/pbr:build N` — it detects existing summaries and skips completed plans
...
2. If cleanup didn't run (hard kill), manually delete stale signal files:
   ...
   ```
3. Run `/pbr:health` to check for orphaned signal files
...
**Solutions**:
1. Plan-Build-Run uses retry loops with exponential backoff for file deletion
2. Plan-Build-Run uses atomic writes (write-to-temp-then-rename) via `atomicWrite()` from `pbr-tools.js`
...
All Plan-Build-Run scripts use `path.join()` for cross-platform compatibility. If you write custom hooks or scripts, never hardcode `/` or `\` in file paths.
...
2. Run `/pbr:health` — Check 8 validates hook logging
3. Verify `plugins/pbr/hooks/hooks.json` hasn't been corrupted
4. Check that `${CLAUDE_PLUGIN_ROOT}` resolves correctly (Claude Code expands this internally)

**Common causes**:
- Missing `node_modules` — run `npm install` in the Plan-Build-Run plugin directory
- Script syntax error — run `node plugins/pbr/scripts/{script}.js` directly to see errors
...
2. Use `/pbr:config` to modify settings interactively — it validates on save
...
- Run `/pbr:help` for the full command reference
- Run `/pbr:health` for automated integrity checks
- Check `.planning/logs/events.jsonl` for workflow event history
- File issues at the Plan-Build-Run repository
