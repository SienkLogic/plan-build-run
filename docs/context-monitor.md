...
| CRITICAL | <= 25% | Stop immediately, save state (`/pbr:pause-work`) |
...
```
Statusline Hook (pbr-statusline.js)
    | writes
    v
/tmp/claude-ctx-{session_id}.json
    ^ reads
    |
Context Monitor (pbr-context-monitor.js, PostToolUse/AfterTool)
    | injects
    v
...
PBR's `/pbr:pause-work` command saves execution state. The WARNING message suggests using it. The CRITICAL message instructs immediate state save.

## Setup

Both hooks are automatically registered during `npx @sienklogic/plan-build-run` installation:
...
```json
{
  "statusLine": {
    "type": "command",
    "command": "node ~/.claude/hooks/pbr-statusline.js"
  },
  "hooks": {
    "PostToolUse": [
      {
        "hooks": [
          {
            "type": "command",
            "command": "node ~/.claude/hooks/pbr-context-monitor.js"
          }
        ]
      }
    ]
    ...
    "AfterTool": [
      {
        "hooks": [
          {
            "type": "command",
            "command": "node ~/.gemini/hooks/pbr-context-monitor.js"
          }
        ]
      }
    ]
