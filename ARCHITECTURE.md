# Towline Architecture — Agent Orchestration
...
```mermaid
flowchart TD
    Start([User runs /dev:begin]) --> Brownfield{Existing code<br/>detected?}

    Brownfield -->|Yes| SuggestScan[Suggest /dev:scan first]
    ...
    subgraph "Parallel Research Fan-Out"
        Researchers[Determine research areas] -->|parallel| R1{{towline-researcher<br/>focus=STACK}}
        Researchers -->|parallel| R2{{towline-researcher<br/>focus=FEATURES}}
        Researchers -->|parallel| R3{{towline-researcher<br/>focus=ARCHITECTURE}}
        Researchers -->|parallel| R4{{towline-researcher<br/>focus=PITFALLS}}
    end
    ...
    ResearchFiles --> Synth{{towline-synthesizer<br/>model=haiku}}
    SkipResearch --> Scope
    ...
    WriteReqs --> Roadmap{{towline-planner<br/>mode=roadmap}}
...
```mermaid
flowchart TD
    Start([User runs /dev:plan N]) --> Validate[Parse and validate phase number]
    ...
    ResearchCheck -->|No| Research{{towline-researcher<br/>phase-scoped}}

    Research --> ResearchOut[(Write phases/NN-slug/RESEARCH.md)]
    ResearchOut --> Planner

    SkipR --> Planner{{towline-planner<br/>mode=phase-plan}}
    ...
    CheckerEnabled -->|Yes| Checker{{towline-plan-checker}}
    ...
    RevisionCount -->|Yes| Revise{{towline-planner<br/>mode=revision}}
...
```mermaid
flowchart TD
    Start([User runs /dev:build N]) --> LoadConfig[("Read config.json,<br/>STATE.md, CONTEXT.md")]
    ...
    subgraph WaveLoop ["Wave Loop (sequential through waves)"]
        ...
        subgraph "Parallel Executors"
            SpawnExec[Spawn executors for wave] -->|parallel| E1{{towline-executor<br/>plan=PLAN-01}}
            SpawnExec -->|parallel| E2{{towline-executor<br/>plan=PLAN-02}}
            SpawnExec -->|parallel| EN{{towline-executor<br/>plan=PLAN-NN}}
        end
        ...
        HandleFail -->|Yes| RetryExec{{towline-executor<br/>retry failed plan}}
    ...
    VerifyCheck -->|Yes| Verifier{{towline-verifier}}
...
flowchart TD
    ...
    AutoFixCheck -->|Yes| Debug{{towline-debugger<br/>analyze gaps}}

    Debug --> GapPlan{{towline-planner<br/>mode=gap-closure}}

    GapPlan --> GapPlanCheck{plan_checking<br/>enabled?}
    GapPlanCheck -->|Yes| GapChecker{{towline-plan-checker}}
...
```mermaid
flowchart TD
    Start([User runs /dev:scan]) --> ExistingCheck{Existing analysis<br/>found?}
...
```mermaid
flowchart TD
    Start([User runs /dev:explore]) --> ProjectExists{.planning/<br/>exists?}
    ...
    UserApproval -->|Yes| Researcher{{towline-researcher<br/>targeted investigation}}
    Researcher --> Continue
...
```mermaid
flowchart TD
    Start([User runs /dev:discuss N]) --> CheckPlans{Plans already<br/>exist for phase?}
...
## `/dev:quick` — Quick Task Execution

Minimal orchestration: one agent, no loops.

```mermaid
flowchart TD
    Start([User runs /dev:quick]) --> CheckProject{.planning/<br/>exists?}
    ...
    ValidateScope -->|No| SuggestPlan[Suggest /dev:plan instead]
    ...
    CreateDir --> Executor{{towline-executor}}
...
## `/dev:continue` — Auto-Resume Next Step

A decision tree that delegates to other skills.

```mermaid
flowchart TD
    Start([User runs /dev:continue]) --> ReadState[(Read STATE.md)]
    ...
    Determine -->|"Phase needs planning"| DelegatePlan["Execute /dev:plan N"]
    Determine -->|"Plans ready, not built"| DelegateBuild["Execute /dev:build N"]
    Determine -->|"Built, not reviewed"| DelegateReview["Execute /dev:review N"]
    Determine -->|"Checkpoint paused"| DelegateBuildResume["Execute /dev:build N<br/>(resume checkpoint)"]
    Determine -->|"All phases complete"| DelegateMilestone["Suggest /dev:milestone"]
...
```mermaid
flowchart TD
    Start([User runs /dev:debug]) --> SessionCheck{Active debug<br/>session exists?}
    ...
    SpawnDebugger{{towline-debugger}}
...
```mermaid
flowchart TD
    Start([User runs /dev:milestone]) --> Sub{Subcommand?}
...
## Session Commands — `/dev:status`, `/dev:pause`, `/dev:resume`

All inline, no agents. Read project state and act on it.

```mermaid
flowchart TD
    subgraph "/dev:status (read-only)"
        S_Start([Run /dev:status]) --> S_Read[("Read STATE.md, ROADMAP.md,<br/>config.json, phase directories")]
    ...
    subgraph "/dev:pause (snapshot)"
        P_Start([Run /dev:pause]) --> P_Read[(Read STATE.md, config.json)]
    ...
    subgraph "/dev:resume (restore)"
        R_Start([Run /dev:resume]) --> R_Read[(Read STATE.md)]
...
## Simple Commands — `/dev:todo`, `/dev:config`, `/dev:health`, `/dev:help`
...
flowchart TD
    subgraph "/dev:todo"
        T_Start([Run /dev:todo]) --> T_Sub{Subcommand?}
    ...
    subgraph "/dev:config"
        C_Start([Run /dev:config]) --> C_Load[(Read config.json)]
    ...
    subgraph "/dev:health"
        H_Start([Run /dev:health]) --> H_Checks["Run 6 checks:<br/>1. Directory structure<br/>2. Config validity<br/>3. Phase consistency<br/>4. Plan/Summary pairing<br/>5. STATE.md accuracy<br/>6. Frontmatter validity"]
        H_Checks --> H_Bonus[Bonus: recent decisions]
        H_Bonus --> H_Report[Present diagnostic report]
    end

    subgraph "/dev:help"
        Help_Start([Run /dev:help]) --> Help_Show[Display command reference]
    end
...
|-------|---------|-------|---------|
| `towline-researcher` | begin, plan, explore | inherit | Domain research, technology investigation |
| `towline-synthesizer` | begin | haiku | Compress research outputs into summary |
| `towline-planner` | begin, plan, review | inherit | Generate roadmaps, phase plans, gap-closure plans |
| `towline-plan-checker` | plan, review | inherit | Verify plan quality, trigger revisions |
| `towline-executor` | build, quick | inherit | Execute plan tasks, write code, create files |
| `towline-verifier` | build, review | inherit | Goal-backward verification of phase outcomes |
| `towline-debugger` | debug, review | inherit | Scientific-method bug investigation |
| `towline-codebase-mapper` | scan | sonnet | Analyze codebase from a specific focus angle |
| `towline-integration-checker` | milestone audit | sonnet | Cross-phase integration and E2E flow checks |

All agents are spawned via `Task()` with `subagent_type` — agent definitions are auto-loaded from `agents/towline-*.md`, never inlined into the orchestrator prompt.
...
### Fan-Out / Fan-In
Used by `/dev:begin` (researchers) and `/dev:scan` (mappers). Multiple agents run in parallel, each writing independent output files. The orchestrator waits for all to complete before proceeding.

### Pipeline
Used by `/dev:plan`. Agents run sequentially — each one's output feeds the next. Researcher → Planner → Checker, with a conditional revision loop back to the planner.

### Wave Execution
Used by `/dev:build`. Plans are grouped into waves by dependency order. Within each wave, executors run in parallel. Waves execute sequentially — wave 2 doesn't start until wave 1 completes.

### Checkpoint Loop
Used by `/dev:debug`. The agent runs, hits a checkpoint when it needs user input, the orchestrator presents findings and collects input, then respawns the agent with the new context.

### Delegation
Used by `/dev:continue`. The orchestrator reads state, decides which command to run next, and hands off to that command's full orchestration flow.
