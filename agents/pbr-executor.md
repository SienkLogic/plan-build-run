...
- Load with: `node ${CLAUDE_PLUGIN_ROOT}/scripts/pbr-tools.cjs config-get gates.auto_checkpoints`
...
- [ ] Requirements marked complete via `requirements mark-complete` (if implements field non-empty)
- [ ] If final plan: `phase complete` called to update ROADMAP.md status and checklist
