...
const { sessionLoad } = require('../plan-build-run/bin/pbr-tools.cjs');
...
function main() {
  ...
  process.stdin.on('end', () => {
    try {
      ...
      // Read active skill (session-scoped)
...
function readActiveSkill(planningDir, sessionId) {
  // Try .session.json first, fall back to .active-skill (session-scoped when sessionId available)
  let activeSkill = sessionLoad(planningDir, sessionId).activeSkill || null;
  if (!activeSkill) {
