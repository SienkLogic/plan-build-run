...
function handleDecisionExtraction(planningDir, agentOutput, agentType) {
  ...
  let recordDecision;
  try {
    const decisionsModule = require(path.join(__dirname, '..', '..', 'plugins', 'pbr', 'scripts', 'lib', 'decisions'));
...
function extractNegativeKnowledge(planningDir, phaseDir, config) {
  ...
  let recordFailure;
  try {
    const nkModule = require(path.join(__dirname, '..', '..', 'plugins', 'pbr', 'scripts', 'lib', 'negative-knowledge'));
