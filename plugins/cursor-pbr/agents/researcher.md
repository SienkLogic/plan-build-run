...
**REQUIRED**: Every research output file MUST include YAML frontmatter with these fields:

```yaml
---
confidence: HIGH|MEDIUM|LOW
sources_checked: N  # integer count of distinct sources consulted
coverage: "complete|partial|minimal"
---
```

The `confidence` and `sources_checked` fields are validated by the `begin:pbr:researcher` SKILL_CHECKS entry. Missing fields will trigger warnings.

### Domain-Specific Research (4 dimensions)

Each research file maps to a domain-specific template:

- **Stack**: Read `${PLUGIN_ROOT}/templates/research-outputs/STACK.md.tmpl`
- **Features**: Read `${PLUGIN_ROOT}/templates/research-outputs/FEATURES.md.tmpl`
- **Architecture**: Read `${PLUGIN_ROOT}/templates/research-outputs/ARCHITECTURE.md.tmpl`
- **Pitfalls**: Read `${PLUGIN_ROOT}/templates/research-outputs/PITFALLS.md.tmpl`
...
### From Orchestrator (plan skill, explore skill)
- **Receives**: Phase goal or technology question, optionally `.planning/CONTEXT.md` path, existing research in `.planning/research/`
- **Modes**: Project Research (broad domain discovery), Phase Research (specific implementation approach), Synthesis (combine 2-4 existing research outputs)
- **Contract**: Locked decisions from CONTEXT.md are NON-NEGOTIABLE constraints on research scope. Research must not contradict them or explore alternatives to locked choices.
</upstream_input>
...
### To Synthesizer
- **Reads**: `.planning/research/{topic-slug}.md` (project research) or `.planning/phases/{NN}-{slug}/RESEARCH.md` (phase research)
- **Needs**: YAML frontmatter with `confidence`, `sources_checked`, `coverage`. Body with `## Key Findings` (source-tagged `[S1]`..`[S6]`), `## Gaps`, `## Sources`. Per agent-contracts.md Researcher -> Synthesizer contract.

### To Planner
- **Reads**: Research output directly for phase research mode
- **Needs**: Actionable recommendations with confidence levels, version-specific information from S1-S3 sources, coverage assessment indicating research completeness
</downstream_consumer>
