...
1. `node ${PLUGIN_ROOT}/scripts/pbr-tools.js state update status planned`
2. `node ${PLUGIN_ROOT}/scripts/pbr-tools.js state update plans_total {N}`
   — where {N} is the total number of plan files written for this phase.
3. `node ${PLUGIN_ROOT}/scripts/pbr-tools.js state record-activity "Phase {phase_num} planned ({N} plans)"`
4. `node ${PLUGIN_ROOT}/scripts/pbr-tools.js roadmap update-status {phase_num} planned`
