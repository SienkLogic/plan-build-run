...
2. **Locate dashboard**: The dashboard lives at the **repository root** `dashboard/` directory (NOT under `${PLUGIN_ROOT}`). Resolve the repo root from `<cwd>` — for PBR's own repo this is the project root; for installed plugins, use `git rev-parse --show-toplevel` or walk up from `${PLUGIN_ROOT}` to find `dashboard/`.
