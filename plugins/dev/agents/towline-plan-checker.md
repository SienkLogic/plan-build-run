...
**Checks**: Scan for contradictions with locked decisions. Scan for deferred idea implementation. Check user constraints (e.g., $0 budget = no paid services). If phase-level CONTEXT.md from `/dev:discuss`, verify all LOCKED decisions addressed. Spot-check research incorporation — key findings reflected or noted as out-of-scope.
...
Reference: `references/agent-interactions.md` — see the towline-plan-checker section for full details on inputs and outputs.
