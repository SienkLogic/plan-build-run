...
- The skill already uses `towline-tools.js state load` (build, plan, review, import)
...
When the skill has a specific topic (e.g., `/dev:explore auth`), scope the briefing:
...
For `/dev:scan`, the pattern adapts to codebase analysis rather than project state:
