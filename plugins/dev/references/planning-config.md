...
Schema, fields, defaults, and behavioral effects of Towline's `config.json`.

---

## Overview

Towline's configuration lives at `.planning/config.json` in the project root. It is created by `/dev:begin` and can be modified via `/dev:config` or by editing the file directly. The config controls workflow behavior, model selection, parallelization, git integration, and confirmation gates.
...
The `version` field tracks schema migrations. Current version is `2`. When loading a config with an older version, Towline runs automatic migration (e.g., v1 to v2 adds missing fields with defaults, renames `model_profile` to per-agent `models` object).
...
Controls how aggressively Towline manages context budget:
...
| `phase_branch_template` | string | `"towline/phase-{phase}-{slug}"` | Phase branch naming |
| `milestone_branch_template` | string | `"towline/{milestone}-{slug}"` | Milestone branch naming |
...
|-------|---------|----------------|
| `confirm_project` | `true` | Before creating a new Towline project (`/dev:begin`) |
| `confirm_roadmap` | `true` | Before finalizing a roadmap |
| `confirm_plan` | `true` | Before finalizing plans for a phase |
| `confirm_execute` | `false` | Before starting phase execution (`/dev:build`) |
...
These cannot be disabled via `/dev:config` -- they require manual editing of `config.json` as a deliberate action.

---

## Validation Rules

When modifying config (via `/dev:config` or direct edit):
