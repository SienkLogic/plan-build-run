...
These anti-patterns apply to ALL Towline agents. Each agent also has role-specific anti-patterns defined in its own agent file.
...
12. **DO NOT** read agent definition files from `agents/towline-*.md`. Agent definitions are auto-loaded by Claude Code via `subagent_type`. Reading them wastes context.
