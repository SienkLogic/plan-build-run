...
Towline's commit conventions, commit points, branching strategy, and hook scripts.
...
Beyond the standard `{phase}-{plan}` scope, Towline recognizes these additional patterns:

| Pattern | When Used | Example |
|---------|-----------|---------|
| `{type}(quick-{NNN})` | Quick-task commits via `/dev:quick` | `feat(quick-001): add health endpoint` |
...
| Merge commits | Git merge operations | `Merge branch 'towline/phase-02-auth'` |
...
Towline defines strict rules about WHEN commits happen:
...
| `phase` | Each phase gets its own branch: `towline/phase-{NN}-{slug}`. Squash-merged to main on completion. |
| `milestone` | Each milestone gets a branch: `towline/{milestone}-{slug}`. Merged on milestone completion. |
...
1. Build orchestrator creates branch: `git checkout -b towline/phase-{NN}-{slug}`
...
4. If confirmed: `git checkout main && git merge --squash towline/phase-{NN}-{slug}`
5. Phase branch is deleted after merge

Branch name templates are configured in `config.json`:
- `git.phase_branch_template`: Default `towline/phase-{phase}-{slug}`
- `git.milestone_branch_template`: Default `towline/{milestone}-{slug}`
...
Towline uses Claude Code hooks (defined in `hooks/hooks.json`) to enforce conventions and track progress during execution. All hooks log via `hook-logger.js` to `.planning/.hook-log` (JSONL format, max 200 entries).
