...
1. `node ${PLUGIN_ROOT}/scripts/pbr-tools.js state advance-plan`
...
2. `node ${PLUGIN_ROOT}/scripts/pbr-tools.js state update-progress`
3. `node ${PLUGIN_ROOT}/scripts/pbr-tools.js state record-activity "Plan {plan_id} complete"`
4. `node ${PLUGIN_ROOT}/scripts/pbr-tools.js roadmap update-plans {phase_num} {completed} {total}`
5. If the plan frontmatter contains a non-empty `implements` array (REQ-IDs), mark those requirements as complete:
   `node ${PLUGIN_ROOT}/scripts/pbr-tools.js requirements mark-complete {comma-separated REQ-IDs}`
...
- Run: `node ${PLUGIN_ROOT}/scripts/pbr-tools.js phase complete {phase_num}`
