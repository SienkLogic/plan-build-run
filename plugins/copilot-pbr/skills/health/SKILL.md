...
**To fix:** Run `/pbr:new-project` to initialize.
```
Stop all further checks.
- PASS: All three required files exist
- FAIL: List each missing file — "Run `/pbr:new-project` to re-initialize, or create the file manually."

**Advisory checks — WARN if missing, do not FAIL:**

- `.planning/PROJECT.md`
  If missing: `⚠ PROJECT.md not found. Run /pbr:new-project to generate it, or create from plugins/pbr/templates/PROJECT.md.tmpl.`
  If present: Check for `## Context` section (merged from legacy CONTEXT.md). If missing: `⚠ PROJECT.md has no ## Context section. Run /pbr:new-project to regenerate, or add manually.`

- `.planning/REQUIREMENTS.md`
  If missing: `⚠ REQUIREMENTS.md not found. Run /pbr:new-project to generate it, or create from plugins/pbr/templates/REQUIREMENTS.md.tmpl.`

**Merged state validation:**

- `.planning/STATE.md` ## History section:
  If STATE.md exists but has no `## History` section: `⚠ STATE.md has no ## History section. History will be tracked once a milestone completes.`

- `.phase-manifest.json` for completed phases:
  For each phase marked as verified/complete in ROADMAP.md, check if `.planning/phases/{NN}-{slug}/.phase-manifest.json` exists.
  If missing: `⚠ Phase {N} has no .phase-manifest.json. Undo operations will fall back to git log scanning.`

**Legacy file detection:**

- If `.planning/CONTEXT.md` exists alongside `PROJECT.md` with `## Context` section: `⚠ Legacy CONTEXT.md detected alongside merged PROJECT.md. Run /pbr:new-project to migrate — the legacy file can be removed.`
- If `.planning/HISTORY.md` exists alongside `STATE.md` with `## History` section: `⚠ Legacy HISTORY.md detected alongside merged STATE.md. Run /pbr:new-project to migrate — the legacy file can be removed.`

These files are generated by /pbr:new-project. Absence is expected on projects created before v2.15.
...
- WARN (missing summary in complete phase): Phase marked complete but plan has no SUMMARY — "Execute with `/pbr:execute-phase` or mark phase incomplete."
...
- WARN (missing fields): Report each missing field and which skill uses it — "Run `/pbr:settings` to set all options."
...
- WARN (orphaned progress files): List each file with its parent phase directory:
  ...
  ```
  Fix suggestion: "These are crash recovery breadcrumbs from interrupted builds. Safe to delete if no `/pbr:execute-phase` is currently running. Remove with `rm <path>`."
- WARN (orphaned checkpoint manifests): List each file:
  ...
  ```
  Fix suggestion: "Checkpoint manifests are leftover from interrupted builds. Safe to delete if no `/pbr:execute-phase` is currently running. Remove with `rm <path>`."
