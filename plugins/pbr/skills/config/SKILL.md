...
First, check for global defaults: `node ${CLAUDE_PLUGIN_ROOT}/scripts/pbr-tools.cjs config load-defaults`
...
If yes, run: `node ${CLAUDE_PLUGIN_ROOT}/scripts/pbr-tools.cjs config save-defaults`
...
To resolve the profile, run: `node ${CLAUDE_PLUGIN_ROOT}/scripts/pbr-tools.cjs config resolve-depth`
...
```bash
node "${CLAUDE_PLUGIN_ROOT}/scripts/pbr-tools.cjs" config save-defaults
```
