 ...
 * Always returns a path under .sessions/{sessionId}/.
 *
 * @param {string} planningDir - Path to .planning/ directory
 * @param {string} filename - Filename to resolve (e.g. '.session.json', '.active-skill')
 * @param {string} sessionId - Session identifier (required)
 * @returns {string} Resolved path
 */
function resolveSessionPath(planningDir, filename, sessionId) {
  return path.join(planningDir, '.sessions', sessionId, filename);
}
 ...
 * @param {string} sessionId - Session identifier (required)
 */
function ensureSessionDir(planningDir, sessionId) {
  const dirPath = path.join(planningDir, '.sessions', sessionId);
 ...
 * @param {string} sessionId - Session identifier (required)
 */
function removeSessionDir(planningDir, sessionId) {
  const dirPath = path.join(planningDir, '.sessions', sessionId);
 ...
 */
function cleanStaleSessions(planningDir) {
  ...
  try {
    ...
    for (const entry of entries) {
      ...
      // Try reading meta.json for creation time
      const metaPath = path.join(dirPath, 'meta.json');
      try {
...
function sessionSave(dir, data, sessionId) {
  ...
  try {
    if (sessionId) {
      ensureSessionDir(dir, sessionId);
    }
 ...
 */
function writeActiveSkill(planningDir, skillName, sessionId) {
  ...
  try {
    ...
    try { sessionSave(planningDir, { activeSkill: skillName }, sessionId); } catch (_e) { /* non-fatal */ }
    ...
    if (e.code === 'EEXIST') {
      ...
      try {
        ...
        if (lockAgeMs > staleThresholdMs) {
          ...
          return writeActiveSkill(planningDir, skillName, sessionId);
        }
      } catch (_statErr) {
 ...
 */
function isClaimStale(claimData, planningDir) {
  const sessionDir = path.join(planningDir, '.sessions', claimData.session_id);
 ...
 */
function acquireClaim(planningDir, phaseDir, sessionId, skill) {
  ...
  if (fs.existsSync(claimPath)) {
    try {
      ...
      if (existing.session_id !== sessionId) {
        const staleCheck = isClaimStale(existing, planningDir);
        if (staleCheck.stale) {
      ...
      // Malformed claim file — remove and proceed
 ...
 */
function releaseClaim(_planningDir, phaseDir, sessionId) {
  const claimPath = path.join(phaseDir, '.claim');
 ...
 */
function listClaims(planningDir) {
  const phasesDir = path.join(planningDir, 'phases');
  if (!fs.existsSync(phasesDir)) {
    return { claims: [] };
  }
    ...
    for (const entry of entries) {
      ...
      try {
        ...
        results.push({
          phase: entry.name,
          ...claimData,
          stale: isClaimStale(claimData, planningDir).stale
        });
      } catch (_e) {
 ...
 */
function releaseSessionClaims(planningDir, sessionId) {
  const phasesDir = path.join(planningDir, 'phases');
  if (!fs.existsSync(phasesDir)) {
    return { released: [] };
  }
