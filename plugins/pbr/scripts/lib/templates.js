...
const BUILT_IN_TEMPLATES = {
  'auth-oauth': {
    ...
    body: [
      '<objective>',
      'Set up {{provider}} OAuth2 authentication with {{session_store}} session management.',
      '</objective>',
      '',
      '<tasks>',
      '',
      ...
      '</action>',
      '<acceptance_criteria>',
      'test -f src/auth/{{provider}}-client.js',
      'test -f src/auth/oauth-config.js',
      'grep -q "clientId" src/auth/oauth-config.js',
      '</acceptance_criteria>',
      '<verify>',
      ...
      'grep -q "{{callback_route}}" src/routes/auth.js',
      'grep -q "{{session_store}}" src/middleware/auth.js',
      '</acceptance_criteria>',
      '<verify>',
      ...
      '',
      '<verification>',
      '1. `node -e "require(\'./src/auth/{{provider}}-client.js\')"` -- OAuth client loads',
      '2. `npm test -- --testPathPattern="auth" --no-coverage` -- auth tests pass',
      '</verification>',
    ].join('\n'),
  },
  ...
  'crud-rest': {
    ...
    body: [
      '<objective>',
      'Create REST API CRUD endpoints for {{resource_name}} with {{db_type}} backend.',
      '</objective>',
      '',
      '<tasks>',
      '',
      ...
      '</action>',
      '<acceptance_criteria>',
      'test -f src/models/{{resource_name}}.js',
      'test -f src/migrations/create-{{resource_name}}.js',
      'grep -q "{{resource_name}}" src/models/{{resource_name}}.js',
      '</acceptance_criteria>',
      '<verify>',
      ...
      '</action>',
      '<acceptance_criteria>',
      'test -f src/routes/{{resource_name}}.js',
      'test -f src/controllers/{{resource_name}}Controller.js',
      'grep -q "router" src/routes/{{resource_name}}.js',
      '</acceptance_criteria>',
      '<verify>',
      ...
      '</action>',
      '<acceptance_criteria>',
      'test -f tests/{{resource_name}}.test.js',
      'grep -q "POST" tests/{{resource_name}}.test.js',
      'grep -q "DELETE" tests/{{resource_name}}.test.js',
      '</acceptance_criteria>',
      '<verify>',
      '<automated>npm test -- --testPathPattern="{{resource_name}}" --no-coverage</automated>',
      '</verify>',
      ...
      '',
      '<verification>',
      '1. `npm test -- --testPathPattern="{{resource_name}}" --no-coverage` -- all CRUD tests pass',
      '</verification>',
    ].join('\n'),
  },
  ...
  'crud-graphql': {
    ...
    body: [
      '<objective>',
      'Create GraphQL schema and resolvers for {{resource_name}} CRUD operations.',
      '</objective>',
      '',
      '<tasks>',
      '',
      ...
      '</action>',
      '<acceptance_criteria>',
      'test -f src/schema/{{resource_name}}.graphql',
      'grep -q "{{resource_name}}" src/schema/{{resource_name}}.graphql',
      'grep -q "Mutation" src/schema/{{resource_name}}.graphql',
      '</acceptance_criteria>',
      '<verify>',
      ...
      '</action>',
      '<acceptance_criteria>',
      'test -f src/resolvers/{{resource_name}}.js',
      'grep -q "get{{resource_name}}" src/resolvers/{{resource_name}}.js',
      'grep -q "delete" src/resolvers/{{resource_name}}.js',
      '</acceptance_criteria>',
      '<verify>',
      '<automated>npm test -- --testPathPattern="{{resource_name}}" --no-coverage</automated>',
      '</verify>',
      ...
      '',
      '<verification>',
      '1. `node -e "require(\'./src/schema/index.js\')"` -- schema loads',
      '2. `npm test -- --testPathPattern="{{resource_name}}" --no-coverage` -- resolver tests pass',
      '</verification>',
    ].join('\n'),
  },
  ...
  'payments-stripe': {
    ...
    body: [
      '<objective>',
      'Implement Stripe checkout and webhook handling for {{product_model}} purchases.',
      '</objective>',
      '',
      '<tasks>',
      '',
      ...
      '</action>',
      '<acceptance_criteria>',
      'test -f src/payments/checkout.js',
      'test -f src/routes/checkout.js',
      'grep -q "create-session" src/routes/checkout.js',
      '</acceptance_criteria>',
      '<verify>',
      ...
      '</action>',
      '<acceptance_criteria>',
      'test -f src/payments/webhook.js',
      'grep -q "{{webhook_path}}" src/routes/webhook.js',
      'grep -q "signature" src/payments/webhook.js',
      '</acceptance_criteria>',
      '<verify>',
      ...
      '',
      '<verification>',
      '1. `node -e "require(\'./src/payments/checkout.js\')"` -- checkout module loads',
      '2. `npm test -- --testPathPattern="webhook" --no-coverage` -- webhook tests pass',
      '</verification>',
    ].join('\n'),
  },
};
