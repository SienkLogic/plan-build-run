...
function planIndex(phaseNum, planningDir) {
  ...
  for (const file of planFiles) {
    ...
    const plan = {
      file,
      plan_id: fm.plan || file.replace(/^PLAN-?/i, '').replace(/\.md$/i, ''),
...
function mustHavesCollect(phaseNum, planningDir) {
  ...
  for (const file of planFiles) {
    const content = fs.readFileSync(path.join(fullDir, file), 'utf8');
    const fm = parseYamlFrontmatter(content);
    const planId = fm.plan || file.replace(/^PLAN-?/i, '').replace(/\.md$/i, '');
...
async function phaseComplete(phaseNum, planningDir) {
  ...
  const roadmapResult = await lockedFileUpdate(roadmapPath, (content) => {
    ...
    if (cols.length >= 4) {
      cols[3] = ` Complete`;
      // Add date — if there are only 4 cols (+ trailing empty), add a new column
      if (cols.length === 5) {
        // Insert date before the trailing empty
 ...
 *   phase: "NN",
 *   completed: "ISO_DATE",
 *   commits: [{ hash, message, plan, task }],
 *   first_commit: "abc123",
 *   last_commit: "def456"
 * }
 *
