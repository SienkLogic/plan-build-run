...
```bash
pbr-tools intel patch-meta .planning/intel/stack.json --cwd <project_root>
```
...
```bash
pbr-tools intel patch-meta .planning/intel/files.json --cwd <project_root>
```
...
```bash
pbr-tools intel patch-meta .planning/intel/apis.json --cwd <project_root>
```
...
```bash
pbr-tools intel patch-meta .planning/intel/deps.json --cwd <project_root>
```
...
Run: `pbr-tools intel validate --cwd <project_root>`
...
Run: `pbr-tools intel snapshot --cwd <project_root>`
...
<success_criteria>
- [ ] Config gate checked (intel.enabled)
...
8. DO NOT skip the config gate check
9. DO NOT consume more than 50% context before producing output -- write incrementally
10. DO NOT read agent .md files from agents/ -- auto-loaded via subagent_type
