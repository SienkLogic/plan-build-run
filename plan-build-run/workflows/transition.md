...
```bash
TRANSITION=$(node "$HOME/.claude/plan-build-run/bin/pbr-tools.cjs" phase complete "${current_phase}")
```
...
**Note:** Basic position updates (Current Phase, Status, Current Plan, Last Activity) were already handled by `pbr-tools phase complete` in the update_roadmap_and_state step.

Verify the updates are correct by reading STATE.md. If the progress bar needs updating, use:

```bash
PROGRESS=$(node "$HOME/.claude/plan-build-run/bin/pbr-tools.cjs" progress bar --raw)
```
...
**Use the transition result from `pbr-tools phase complete`:**
...
Exit skill and invoke SlashCommand("/pbr:plan-phase [X+1] --auto")
...
Exit skill and invoke SlashCommand("/pbr:discuss-phase [X+1] --auto")
...
`/pbr:discuss-phase [X+1]` — gather context and clarify approach
...
**Also available:**
- `/pbr:plan-phase [X+1]` — skip discussion, plan directly
- `/pbr:research-phase [X+1]` — investigate unknowns
...
**Also available:**
- `/pbr:discuss-phase [X+1]` — revisit context
- `/pbr:research-phase [X+1]` — investigate unknowns
...
```bash
node "$HOME/.claude/plan-build-run/bin/pbr-tools.cjs" config-set workflow._auto_chain_active false
```
...
Exit skill and invoke SlashCommand("/pbr:complete-milestone {version}")
