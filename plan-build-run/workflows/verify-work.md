...
**Also available:**
- `/pbr:execute-phase {phase} --gaps-only` — if fix plans already exist
- Re-run `/pbr:verify-work {phase}` — test additional scenarios

───────────────────────────────────────────────────────────────
```

**STOP HERE.** Do not spawn planners or checkers. The user will `/clear` and run `/pbr:plan-phase --gaps` in a fresh context where the planner gets full context budget for quality gap closure plans.
</step>
...
@~/.claude/plan-build-run/templates/crud-flow-verification.md
</integration_health>
...
- [ ] If issues: user directed to `/pbr:plan-phase --gaps` in fresh context
