...
<required_reading>
@~/.claude/plan-build-run/references/verification-patterns.md
@~/.claude/plan-build-run/templates/verification-report.md
</required_reading>
...
Use pbr-tools to extract must_haves from each PLAN:

```bash
for plan in "$PHASE_DIR"/*-PLAN.md; do
  MUST_HAVES=$(node "$HOME/.claude/plan-build-run/bin/pbr-tools.cjs" frontmatter get "$plan" --field must_haves)
...
```bash
PHASE_DATA=$(node "$HOME/.claude/plan-build-run/bin/pbr-tools.cjs" roadmap get-phase "${phase_number}" --raw)
```
...
Use pbr-tools for artifact verification against must_haves in each PLAN:

```bash
for plan in "$PHASE_DIR"/*-PLAN.md; do
  ARTIFACT_RESULT=$(node "$HOME/.claude/plan-build-run/bin/pbr-tools.cjs" verify artifacts "$plan")
...
Use pbr-tools for key link verification against must_haves in each PLAN:

```bash
for plan in "$PHASE_DIR"/*-PLAN.md; do
  LINKS_RESULT=$(node "$HOME/.claude/plan-build-run/bin/pbr-tools.cjs" verify key-links "$plan")
...
See ~/.claude/plan-build-run/templates/verification-report.md for complete template.
</step>
