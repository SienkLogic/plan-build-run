...
Detect whether PBR is installed locally or globally by checking both locations:

```bash
# Check local first (takes priority)
if [ -f "./.claude/plan-build-run/VERSION" ]; then
  cat "./.claude/plan-build-run/VERSION"
  echo "LOCAL"
elif [ -f ~/.claude/plan-build-run/VERSION ]; then
  cat ~/.claude/plan-build-run/VERSION
  echo "GLOBAL"
else
  echo "UNKNOWN"
...
```bash
npm view plan-build-run-cc version 2>/dev/null
```
...
To update manually: `npx plan-build-run-cc --global`
```
...
**If LOCAL install:**
```bash
npx plan-build-run-cc --local
```

**If GLOBAL install (or unknown):**
```bash
npx plan-build-run-cc --global
```
...
**If LOCAL install:**
```bash
rm -f ./.claude/cache/pbr-update-check.json
```
...
[View full changelog](https://github.com/davesienkowski/plan-build-run/blob/main/CHANGELOG.md)
```
</step>
