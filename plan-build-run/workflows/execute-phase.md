...
2. **Spawn executor agents:**
   ...
   Task(
     ...
     prompt="
       ...
       - Context: {CONTEXT_FILE} (if exists — contains user decisions from discuss-phase)
       </files_to_read>

       <user_decisions>
       If a CONTEXT.md file exists for this phase, read it FIRST. It contains
       user decisions from the discuss-phase that MUST be honored during execution.
       The plan encodes most decisions, but CONTEXT.md is the authoritative source
       for user intent. When the plan is ambiguous, defer to CONTEXT.md.
       </user_decisions>
...
Route to next action based on phase complete result.

**Use `is_last_phase`, `next_phase`, and `next_phase_name` from the `phase complete` result.**

---

**If `is_last_phase` is false (more phases remain):**
...
`/pbr:discuss-phase {next_phase}` — gather context and clarify approach
...
**Also available:**
- `/pbr:plan-phase {next_phase}` — skip discussion, plan directly
- `/pbr:verify-work {X}` — user acceptance test before continuing
- `/pbr:transition` — full transition with PROJECT.md evolution
...
**Also available:**
- `/pbr:discuss-phase {next_phase}` — update existing context
- `/pbr:verify-work {X}` — user acceptance test before continuing
- `/pbr:transition` — full transition with PROJECT.md evolution
