...
@~/.claude/plan-build-run/references/git-integration.md
</required_reading>
...
```bash
INIT=$(node ~/.claude/plan-build-run/scripts/pbr-tools.cjs init execute-phase "${PHASE}" --include state,config)
```
...
**Pattern A:** init_agent_tracking → spawn Task(subagent_type="pbr-executor", model=executor_model) with prompt: execute plan at [path], autonomous, all tasks + SUMMARY + commit, follow deviation/auth rules, report: plan name, tasks, SUMMARY path, commit hash → track agent_id → wait → update tracking → report.
...
2. Per segment:
   - Subagent route: spawn pbr-executor for assigned tasks only. Prompt: task range, plan path, read full plan for context, execute assigned tasks, track deviations, NO SUMMARY/commit. Track via agent protocol.
...
See `~/.claude/plan-build-run/references/tdd.md` for structure.
</tdd_plan_execution>
...
See ~/.claude/plan-build-run/references/checkpoints.md for details.
</step>
...
If user_setup exists: create `{phase}-USER-SETUP.md` using template `~/.claude/plan-build-run/templates/user-setup.md`. Per service: env vars table, account setup checklist, dashboard config, local dev notes, verification commands. Status "Incomplete". Set `USER_SETUP_CREATED=true`. If empty/missing: skip.
</step>

<step name="create_summary">
Create `{phase}-{plan}-SUMMARY.md` at `.planning/phases/XX-name/`. Use `~/.claude/plan-build-run/templates/summary.md`.
...
Update STATE.md using pbr-tools:

```bash
# Advance plan counter (handles last-plan edge case)
node ~/.claude/plan-build-run/scripts/pbr-tools.cjs state advance-plan

# Recalculate progress bar from disk state
node ~/.claude/plan-build-run/scripts/pbr-tools.cjs state update-progress

# Record execution metrics
node ~/.claude/plan-build-run/scripts/pbr-tools.cjs state record-metric \
...
node ~/.claude/plan-build-run/scripts/pbr-tools.cjs state add-decision \
  --phase "${PHASE}" --summary "${DECISION_TEXT}" --rationale "${RATIONALE}"

# Add blockers if any found
node ~/.claude/plan-build-run/scripts/pbr-tools.cjs state add-blocker "Blocker description"
```
</step>

<step name="update_session_continuity">
Update session info using pbr-tools:
...
```bash
node ~/.claude/plan-build-run/scripts/pbr-tools.cjs commit "docs({phase}-{plan}): complete [plan-name] plan" --files .planning/phases/XX-name/{phase}-{plan}-SUMMARY.md .planning/STATE.md .planning/ROADMAP.md
```
</step>
...
```bash
node ~/.claude/plan-build-run/scripts/pbr-tools.cjs commit "" --files .planning/codebase/*.md --amend
```
</step>
...
|-----------|-------|--------|
| summaries < plans | **A: More plans** | Find next PLAN without SUMMARY. Yolo: auto-continue. Interactive: show next plan, suggest `/pbr:execute-phase {phase}` + `/pbr:verify-work`. STOP here. |
| summaries = plans, current < highest phase | **B: Phase done** | Show completion, suggest `/pbr:plan-phase {Z+1}` + `/pbr:verify-work {Z}` + `/pbr:discuss-phase {Z+1}` |
| summaries = plans, current = highest phase | **C: Milestone done** | Show banner, suggest `/pbr:complete-milestone` + `/pbr:verify-work` + `/pbr:add-phase` |
