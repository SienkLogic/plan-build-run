...
# config settings can be fetched via pbr-tools config-get if needed
```
...
```bash
node "$HOME/.claude/plan-build-run/bin/pbr-tools.cjs" phases list --type summaries --raw
...
node "$HOME/.claude/plan-build-run/bin/pbr-tools.cjs" state add-blocker --text-file "${BLOCKER_TEXT_FILE}"
```
</step>
...
```bash
node "$HOME/.claude/plan-build-run/bin/pbr-tools.cjs" roadmap update-plan-progress "${PHASE}"
```
...
```bash
node "$HOME/.claude/plan-build-run/bin/pbr-tools.cjs" commit "" --files .planning/codebase/*.md --amend
```
</step>
