...
```bash
INIT=$(node ~/.claude/plan-build-run/scripts/pbr-tools.cjs init plan-phase "$PHASE" --include state,roadmap,requirements,context,research,verification,uat)
```
...
```bash
PHASE_INFO=$(node ~/.claude/plan-build-run/scripts/pbr-tools.cjs roadmap get-phase "${PHASE}")
```
...
```bash
PHASE_DESC=$(node ~/.claude/plan-build-run/scripts/pbr-tools.cjs roadmap get-phase "${PHASE}" | jq -r '.section')
...
STATE_SNAP=$(node ~/.claude/plan-build-run/scripts/pbr-tools.cjs state-snapshot)
...
<phase_context>
IMPORTANT: If CONTEXT.md exists below, it contains user decisions from /pbr:discuss-phase.
...
```
Task(
  prompt="First, read ~/.claude/agents/pbr-phase-researcher.md for your role and instructions.\n\n" + research_prompt,
...
**Phase Context:**
IMPORTANT: If context exists below, it contains USER DECISIONS from /pbr:discuss-phase.
...
```
Task(
  prompt="First, read ~/.claude/agents/pbr-planner.md for your role and instructions.\n\n" + filled_prompt,
  subagent_type="general-purpose",
  model="{planner_model}",
