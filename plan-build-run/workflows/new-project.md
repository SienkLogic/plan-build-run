...
Auto mode requires an idea document via @ reference (e.g., `/pbr:new-project --auto @prd.md`). If no document provided, error:

```
Error: --auto requires an idea document via @ reference.

Usage: /pbr:new-project --auto @your-idea.md
...
Auto-select all recommended settings without asking:
...
Skip to config creation below.

</if>

**Single question — recommended settings shortcut:**

```
questions: [
  {
    header: "Settings",
    question: "Use recommended workflow settings?",
    multiSelect: false,
    options: [
      { label: "Yes, use recommended (Recommended)", description: "YOLO mode, standard depth, parallel execution, all agents enabled, balanced models" },
      { label: "Customize", description: "Choose each setting individually (8 questions)" }
    ]
  }
]
```

**If "Yes, use recommended":** Apply defaults:
- Mode: yolo
- Depth: standard
- Parallelization: true
- Commit docs: true
...
**If "Customize":** Ask all 8 questions in 2 rounds:
...
**If YOLO mode:** Default to "Research first" without asking.

**If `workflow.research` was set in config (from step 5):** Use that value directly without re-asking. If `workflow.research: true` → proceed to research. If `workflow.research: false` → skip to requirements.
...
Spawn 4 parallel pbr-project-researcher agents with rich context:

```
Task(prompt="First, read ~/.claude/agents/pbr-project-researcher.md for your role and instructions.
...
Spawn pbr-roadmapper agent with context:
...
**If YOLO mode:** Skip approval gate — auto-approve and commit directly. Research was done, requirements were defined, roadmapper produced the phases — auto-approve and proceed.
