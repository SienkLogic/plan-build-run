...
```bash
INIT=$(node "$HOME/.claude/plan-build-run/bin/pbr-tools.cjs" init milestone-op)
...
```bash
integration_checker_model=$(node "$HOME/.claude/plan-build-run/bin/pbr-tools.cjs" resolve-model pbr-integration-checker --raw)
```
...
node "$HOME/.claude/plan-build-run/bin/pbr-tools.cjs" phases list
```
...
PHASE_INFO=$(node "$HOME/.claude/plan-build-run/bin/pbr-tools.cjs" find-phase 01 --raw)
...
Verify cross-phase wiring and E2E user flows.",
  subagent_type="pbr-integration-checker",
...
for summary in .planning/phases/*-*/*-SUMMARY.md; do
  node "$HOME/.claude/plan-build-run/bin/pbr-tools.cjs" summary-extract "$summary" --fields requirements_completed | jq -r '.requirements_completed'
done
```
...
```bash
NYQUIST_CONFIG=$(node "$HOME/.claude/plan-build-run/bin/pbr-tools.cjs" config get workflow.nyquist_validation --raw 2>/dev/null)
```
...
Discovery only — never auto-calls `/pbr:validate-phase`.
...
|-------|---------------|-----------|--------|
| {phase} | exists/missing | true/false/partial | `/pbr:validate-phase {N}` |

Phases needing validation: run `/pbr:validate-phase {N}` for each flagged phase.
...
- /pbr:complete-milestone {version} — proceed anyway (accept tech debt)
