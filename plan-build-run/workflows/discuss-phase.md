...
**Philosophy: Ask questions, then check.**

Check config for `workflow.max_discuss_questions` (default: 4). Ask that many questions per area before offering to continue or move on. Each answer often reveals the next question.
...
2. **Ask questions using AskUserQuestion (up to max_discuss_questions, default 4):**
   - header: "[Area]"
   - question: Specific decision for this area
   - options: 2-3 concrete choices (AskUserQuestion adds "Other" automatically)
   - Include "You decide" as an option when reasonable — captures Claude discretion

3. **After reaching the question limit, check:**
   - header: "[Area]"
   - question: "More questions about [area], or move to next?"
   - options: "More questions" / "Next area"

   If "More questions" → ask another batch, then check again
   If "Next area" → proceed to next selected area

4. **After all areas complete:**

   **If only one area was selected:** Skip this prompt — go directly to `write_context`.

   **If multiple areas were selected:**
   - header: "Done"
