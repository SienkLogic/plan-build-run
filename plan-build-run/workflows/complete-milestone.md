...
2. **Run audit first** — `/pbr:audit-milestone` to assess gap severity
...
for summary in .planning/phases/*-*/*-SUMMARY.md; do
  node "$HOME/.claude/plan-build-run/bin/pbr-tools.cjs" summary-extract "$summary" --fields one_liner | jq -r '.one_liner'
done
```
...
**Note:** MILESTONES.md entry is now created automatically by `pbr-tools milestone complete` in the archive_milestone step. The entry includes version, date, phase/plan/task counts, and accomplishments extracted from SUMMARY.md files.
...
```bash
ARCHIVE=$(node "$HOME/.claude/plan-build-run/bin/pbr-tools.cjs" milestone complete "v[X.Y]" --name "[Milestone Name]")
```
...
If "Skip": Phase directories remain in `.planning/phases/` as raw execution history. Use `/pbr:cleanup` later to archive retroactively.
...
**If doesn't exist:** Create from template at `~/.claude/plan-build-run/templates/retrospective.md`.
...
**Commit:**
```bash
node "$HOME/.claude/plan-build-run/bin/pbr-tools.cjs" commit "docs: update retrospective for v${VERSION}" --files .planning/RETROSPECTIVE.md
```
...
```bash
INIT=$(node "$HOME/.claude/plan-build-run/bin/pbr-tools.cjs" init execute-phase "1")
...
```bash
node "$HOME/.claude/plan-build-run/bin/pbr-tools.cjs" commit "chore: complete v[X.Y] milestone" --files .planning/milestones/v[X.Y]-ROADMAP.md .planning/milestones/v[X.Y]-REQUIREMENTS.md .planning/milestones/v[X.Y]-MILESTONE-AUDIT.md .planning/MILESTONES.md .planning/PROJECT.md .planning/STATE.md
```
```
...
- [ ] User knows next step (/pbr:new-milestone)
