...
function checkActiveSkillIntegrity(data) {
  ...
  const activeSkillFile = sessionId
    ? resolveSessionPath(planningDir, '.active-skill', sessionId)
    : path.join(planningDir, '.active-skill');
  if (!fs.existsSync(activeSkillFile)) {
    return 'Active-skill integrity: .active-skill not found for this session. Skill-specific enforcement is disabled. The invoking skill should write this file. To fix: Wait for the current skill to finish, or delete .active-skill if stale.';
  }
