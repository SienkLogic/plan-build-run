...
/**
 * lib/build.cjs — Build helper functions for Plan-Build-Run orchestrator.
 *
...
/**
 * Resolve the current session ID from .session.json or generate a fallback.
 * @param {string} planningDir
 * @returns {string|null}
 */
function _resolveSessionId(planningDir) {
  try {
...
function stalenessCheck(phaseSlug, planningDir) {
  ...
  for (const planFile of planFiles) {
    ...
    if (fpMatch) {
      ...
      // Mode 2: timestamp-based fallback
...
function checkpointInit(phaseSlug, plans, planningDir) {
  ...
  // Include session_id for multi-session validation
  const sessionId = _resolveSessionId(pd);
...
function checkpointUpdate(phaseSlug, opts, planningDir) {
  ...
  const { wave, resolved, sha, force } = opts || {};

  // Session ID match check: warn if checkpoint was created by a different session
  const currentSessionId = _resolveSessionId(pd);
  if (manifest.session_id && currentSessionId && manifest.session_id !== currentSessionId && !force) {
    return {
      error: 'Checkpoint was created by a different session (manifest: ' +
        manifest.session_id + ', current: ' + currentSessionId +
