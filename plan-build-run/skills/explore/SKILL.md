...
```bash
node {resolved_plugin_root}/bin/pbr-tools.cjs learnings query --tags "stack,tech" 2>/dev/null
```

If non-empty JSON array returned:

- Write to temp file: `node {resolved_plugin_root}/bin/pbr-tools.cjs learnings query --tags "stack,tech" > /tmp/pbr-learnings-$$.md`
