...
```bash
node "${CLAUDE_PLUGIN_ROOT}/bin/pbr-tools.cjs" init execute-phase {phase_number}
```
...
```bash
node "${CLAUDE_PLUGIN_ROOT}/bin/pbr-tools.cjs" frontmatter .planning/phases/{NN}-{slug}/SUMMARY.md
```
