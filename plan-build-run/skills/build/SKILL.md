...
**Skip if** `features.multi_layer_validation` is `false` (default) or not set in config.

When enabled:

1. Read `config.validation_passes` from `.planning/config.json` — default `["correctness", "security"]`.
2. Collect the git diff of all changes made in this wave: `git diff {pre_build_commit}..HEAD`
3. For each pass name in `validation_passes`, spawn a parallel review Task():
   ```
   Task({
     subagent_type: "pbr:general",
     model: "haiku",
     run_in_background: true,
     prompt: "<validation prompt for this pass>\n\nPass: {name}\nFocus: {focus}\n\nDiff:\n{git diff output}\n\nFiles modified: {list}\n\nReturn JSON: { pass: '{name}', findings: [{file, line, severity, message}], summary: '...' }"
   })
   ```
   Focus areas per pass: correctness (logic errors, edge cases, null handling), security (injection, auth bypass, OWASP Top 10), performance (O(n²) loops, allocations, blocking I/O), style (naming, organization), tests (coverage, assertions), accessibility (ARIA, keyboard nav), docs (JSDoc, README), deps (outdated, CVEs).
4. Wait for all parallel review Tasks to complete.
5. Aggregate results:
   - Count findings by severity: high, medium, low
   - Display: "{N} validation passes complete. {M} findings ({H} high, {M2} medium, {L} low)"
   - If any high-severity findings: show advisory warning box — do NOT block (advisory in v1)
6. Log audit evidence to `.planning/logs/hooks.jsonl`:
   ```json
   { "event": "validation_pass", "timestamp": "{ISO}", "passes": [{pass, findingCount, severity}], "blocked": false }
   ```
...
**Skip if** `features.security_scanning` is `false`. (Default: `true` — runs by default.)

When enabled:

1. Get the list of changed files: `git diff --name-only {pre_build_commit}..HEAD`
2. For each changed file, check content against these OWASP-inspired patterns:
   - **Hardcoded secrets** (SEC-001, high): API keys/tokens in string literals — pattern: `(?:api[_-]?key|secret|password|token)\s*[=:]\s*["'][^"']{8,}["']`
   - **eval() usage** (SEC-002, high): `eval(` in non-test code
   - **Shell injection** (SEC-003, high): `exec/spawn` with string concatenation
   - **Path traversal** (SEC-005, medium): `path.join/resolve` with user input variables
   - Skip binary files (.png, .jpg, .woff, etc.)
   - Downgrade severity to "info" for findings in test files (`*.test.js`, `*.spec.js`)
3. Display scan results: "Security scan: {N} files scanned, {M} findings"
   - If high-severity findings: show advisory warning box — do NOT block (advisory in v1)
4. Log audit evidence to `.planning/logs/hooks.jsonl`:
   ```json
   { "event": "security_scan", "timestamp": "{ISO}", "scanned": N, "findings": M, "high": H }
   ```
...
**Skip if** `features.regression_prevention` is `false`. When disabled: use existing full test suite behavior.

When enabled:

1. Get changed files from this wave: `git diff --name-only {pre_build_commit}..HEAD`
2. Apply naming heuristics to select targeted tests:
...
3. **If any config file changed** (`config-schema.json`, `config.json`, `package.json`): use full suite instead
4. Filter candidates to only existing test files
5. Display: "Smart test selection: {N} test files selected (of {T} total)"
6. Use the selected test files when running verification in Step 7 instead of the full suite:
   - Selected: `npx jest {file1} {file2} ...`
   - Full suite: `npx jest`
7. Log audit evidence to `.planning/logs/hooks.jsonl`:
   ```json
   { "event": "test_selection", "timestamp": "{ISO}", "selected": N, "total": T, "changedFiles": [...] }
   ```
