...
> **Note:** Planning methodology is in `agents/pbr-planner.md`.
...
<!-- For checkpoint task examples and patterns, see @~/.claude/plan-build-run/references/checkpoints.md -->
...
**Wave is pre-computed:** Wave numbers are assigned during `/pbr:plan-phase`. Execute-phase reads `wave` directly from frontmatter and groups plans by wave number. No runtime dependency analysis needed.
...
See `~/.claude/plan-build-run/references/tdd.md` for TDD plan structure.
...
See `~/.claude/plan-build-run/templates/user-setup.md` for full schema and examples
...
See `~/.claude/plan-build-run/workflows/verify-phase.md` for verification logic.
