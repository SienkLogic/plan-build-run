...
```bash
PHASE_INFO=$(node "${CLAUDE_PLUGIN_ROOT}/scripts/pbr-tools.js" find-phase "${PHASE}")
```
...
```bash
PHASE_CHECK=$(node "${CLAUDE_PLUGIN_ROOT}/scripts/pbr-tools.js" roadmap get-phase "${PHASE}")
...
```bash
PHASE_DIR=$(node "${CLAUDE_PLUGIN_ROOT}/scripts/pbr-tools.js" find-phase "${PHASE}" --raw)
```
