...
```bash
PHASE_INFO=$(node "${CLAUDE_PLUGIN_ROOT}/scripts/pbr-tools.cjs" find-phase "${PHASE}")
```
...
```bash
PHASE_CHECK=$(node "${CLAUDE_PLUGIN_ROOT}/scripts/pbr-tools.cjs" roadmap get-phase "${PHASE}")
...
```bash
PHASE_DIR=$(node "${CLAUDE_PLUGIN_ROOT}/scripts/pbr-tools.cjs" find-phase "${PHASE}" --raw)
```
