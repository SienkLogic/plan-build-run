...
describe('config-ensure-section command', () => {
  ...
  test('creates config.json with expected structure and types', () => {
    ...
    // Verify structure and types — exact values may vary if ~/.pbr/defaults.json exists
  ...
  test('is idempotent — returns already_exists on second call', () => {
    ...
    const second = runPbrTools('config-ensure-section', tmpDir);
  ...
  test('detects Brave Search from file-based key', () => {
    ...
    // Create .pbr dir and brave_api_key file
    const pbrDirExisted = fs.existsSync(pbrDir);
    try {
      if (!pbrDirExisted) {
        fs.mkdirSync(pbrDir, { recursive: true });
      }
      ...
      if (!pbrDirExisted) {
        try { fs.rmdirSync(pbrDir); } catch { /* ignore if not empty */ }
      }
    }
  });
  ...
  test('merges user defaults from defaults.json', () => {
    ...
    try {
      ...
      if (!pbrDirExisted) {
        try { fs.rmdirSync(pbrDir); } catch { /* ignore */ }
      }
    }
  });
...
describe('config-set command', () => {
  ...
  beforeEach(() => {
    ...
    runPbrTools('config-ensure-section', tmpDir);
  });
  ...
  test('auto-creates nested objects for deep dot-notation', () => {
    ...
    const result = runPbrTools('config-set a.b.c deep_value', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
