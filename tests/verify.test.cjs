...
const { runPbrTools, createTempProject, createTempGitProject, cleanup } = require('./helpers.cjs');
...
describe('validate consistency command', () => {
  ...
  test('passes for consistent project', () => {
    ...
    const result = runPbrTools('validate consistency', tmpDir);
...
describe('verify plan-structure command', () => {
  ...
  test('reports missing required frontmatter fields', () => {
    ...
    const result = runPbrTools('verify plan-structure .planning/phases/01-test/01-01-PLAN.md', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
...
describe('verify phase-completeness command', () => {
  ...
  test('reports complete phase with matching plans and summaries', () => {
    ...
    const result = runPbrTools('verify phase-completeness 01', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
...
describe('verify summary command', () => {
  ...
  test('passes for valid summary with real files and commits', () => {
    ...
    const result = runPbrTools('verify-summary .planning/phases/01-test/01-01-SUMMARY.md', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('respects checkFileCount parameter', () => {
    ...
    const result = runPbrTools('verify-summary .planning/phases/01-test/01-01-SUMMARY.md --check-count 1', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
...
describe('verify references command', () => {
  ...
  test('reports valid when all referenced files exist', () => {
    ...
    const result = runPbrTools('verify references .planning/phases/01-test/doc.md', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
...
describe('verify artifacts command', () => {
  ...
  test('passes when all artifacts exist and match criteria', () => {
    ...
    const result = runPbrTools('verify artifacts .planning/phases/01-test/01-01-PLAN.md', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
...
describe('verify key-links command', () => {
  ...
  test('verifies link when pattern found in source', () => {
    ...
    const result = runPbrTools('verify key-links .planning/phases/01-test/01-01-PLAN.md', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
