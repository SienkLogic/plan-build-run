...
describe('execGit', () => {
  ...
  test('returns non-zero for invalid git command', () => {
...
describe('validateObject', () => {
  test('handles basic schema validation', () => {
    const errors = [];
    const warnings = [];
...
describe('findPhaseInternal', () => {
  test('returns not-found when no phases dir', () => {
  ...
  test('finds phase directory by number', () => {
...
describe('getMilestoneInfo', () => {
  test('returns milestone info from STATE.md', () => {
    const planningDir = path.join(tmpDir, '.planning');
    fs.mkdirSync(path.join(planningDir, 'logs'), { recursive: true });
    fs.writeFileSync(path.join(planningDir, 'STATE.md'),
      '---\ncurrent_phase: 1\n---\n## Milestone\nCurrent: v1.0\nPhases: 1-5');
  ...
  test('returns null or empty when no STATE.md', () => {
    const result = getMilestoneInfo(tmpDir);
...
describe('session management', () => {
  ...
  test('cleanStaleSessions removes old sessions', () => {
...
describe('KNOWN_AGENTS', () => {
  test('includes core agent types', () => {
