...
describe('hooks.json schema compliance', () => {
  ...
  test('has required top-level properties', async () => {
    expect(hooks).toHaveProperty('hooks');
    expect(typeof hooks.hooks).toBe('object');
  });

  test('has $schema reference', async () => {
  ...
  test('every hook has valid type and required fields', async () => {
    const violations = [];
    for (const [event, entries] of Object.entries(hooks.hooks)) {
      for (let i = 0; i < entries.length; i++) {
  ...
  test('matcher is always a string when present', async () => {
    const violations = [];
    for (const [event, entries] of Object.entries(hooks.hooks)) {
      for (let i = 0; i < entries.length; i++) {
  ...
  test('async hooks always have a numeric timeout', async () => {
    const violations = [];
    for (const [event, entries] of Object.entries(hooks.hooks)) {
  ...
  test('all commands reference CLAUDE_PLUGIN_ROOT for script paths', async () => {
    const violations = [];
    for (const [event, entries] of Object.entries(hooks.hooks)) {
      for (const entry of entries) {
        for (const hook of entry.hooks || []) {
  ...
  test('no duplicate commands in same event+matcher', async () => {
    const seen = new Map();
    const duplicates = [];
    for (const [event, entries] of Object.entries(hooks.hooks)) {
      for (const entry of entries) {
...
describe.skip('hooks-schema.json is valid JSON Schema', () => {
  test('has required JSON Schema fields', async () => {});
  test('defines all hook event types', async () => {});
  test('hookCommand definition covers all used fields', async () => {});
});
...
describe('config-schema.json is valid JSON Schema', () => {
  ...
  test('defines all major config sections', async () => {
  ...
  test('features section covers all known toggles', async () => {
  ...
  test('hooks section covers all known hook toggles', async () => {
  ...
  test('all sections disallow additional properties', async () => {
...
describe.skip('plugin.json manifest constraints', () => {
  ...
  test('does NOT declare hooks field (auto-loaded from hooks/)', async () => {
    expect(plugin).not.toHaveProperty('hooks');
  });

  test('has mandatory version field', async () => {
    expect(plugin.version).toBeDefined();
    expect(typeof plugin.version).toBe('string');
    expect(plugin.version).toMatch(/^\d+\.\d+\.\d+/);
  });

  test('has mandatory name field', async () => {
    expect(plugin.name).toBeDefined();
    expect(typeof plugin.name).toBe('string');
  });

  test('component fields are arrays when present', async () => {
