...
describe('checkBoundary unit tests', () => {
  ...
  test('returns null for files not under .planning/phases/', async () => {
    expect(checkBoundary({ tool_input: { file_path: '/some/src/app.js' } })).toBeNull();
  });

  test('returns null when no phase number in path', async () => {
  ...
  test('returns null when file phase matches current phase', async () => {
    fs.writeFileSync(path.join(planningDir, 'STATE.md'), 'Phase: 2 of 5');
    const filePath = path.join(planningDir, 'phases', '02-auth', 'PLAN.md');
    expect(checkBoundary({ tool_input: { file_path: filePath } })).toBeNull();
  });

  test('returns advisory (exit 0) for cross-phase write without enforcement', async () => {
  ...
  test('returns block (exit 2) for cross-phase write with enforcement', async () => {
    fs.writeFileSync(path.join(planningDir, 'STATE.md'), 'Phase: 1 of 5');
...
describe('getEnforceSetting', () => {
  test('returns false when config.json missing', async () => {
  ...
  test('returns false when safety section missing', async () => {
