/**
 * Tests for plan-build-run/bin/lib/migrate.js
...
describe('getMigrationPath', () => {
  test('correct sequence v0 to v1', async () => {
  ...
  test('returns empty array if already at current version', async () => {
    const path_ = migrate.getMigrationPath(1, 1);
    expect(path_).toEqual([]);
  });

  test('throws on downgrade attempt', async () => {
    expect(() => migrate.getMigrationPath(2, 1)).toThrow();
  });

  test('returns empty array for equal versions', async () => {
...
describe('CURRENT_SCHEMA_VERSION', () => {
  test('is numeric and at least 1', async () => {
...
// --- MIGRATIONS array ---
describe('MIGRATIONS', () => {
  test('has at least one entry', async () => {
  ...
  test('v0->v1 migration adds schema_version', async () => {
