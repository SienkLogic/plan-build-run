...
const { detectDrift, initExecutePhase, initPlanPhase, initQuick, initVerifyWork, initResume, initProgress } = require('../plugins/pbr/scripts/lib/init');
const { statePatch, stateAdvancePlan, stateRecordMetric } = require('../plugins/pbr/scripts/lib/state');
...
describe('pbr-tools compound init commands', () => {
  ...
  describe('initExecutePhase', () => {
    ...
    test('returns error when no .planning dir', async () => {
      fs.rmSync(path.join(tmpDir, '.planning'), { recursive: true });
      var result = initExecutePhase('3');
      expect(result.error).toBeDefined();
    });

    test('initExecutePhase respects overrideModel when provided', async () => {
    ...
    test('initExecutePhase falls back to config model when overrideModel is undefined', async () => {
  ...
  describe('initPlanPhase', () => {
    ...
    test('existing_artifacts lists markdown files in phase dir', async () => {
      var result = initPlanPhase('3');
      expect(Array.isArray(result.existing_artifacts)).toBe(true);
      expect(result.existing_artifacts).toContain('01-PLAN.md');
    });

    test('phase goal comes from ROADMAP.md', async () => {
      var result = initPlanPhase('3');
      expect(result.phase.goal || null).toBeNull();
    });

    test('workflow reflects feature flags', async () => {
      var result = initPlanPhase('3');
    ...
    test('initPlanPhase respects overrideModel when provided', async () => {
    ...
    test('initPlanPhase falls back to config model when overrideModel is undefined', async () => {
  ...
  describe('initQuick', () => {
    ...
    test('next_task_number is 001 when quick/ is empty', async () => {
      var result = initQuick('first task');
      expect(result.next_task_number).toBe('001');
    });

    test('slug is derived from description', async () => {
      var result = initQuick('Fix Login Bug');
      expect(result.slug).toBe('fix-login-bug');
    });

    test('dir uses relative .planning/quick/ path', async () => {
      var result = initQuick('my task');
      expect(result.dir).toContain('.planning');
      expect(result.dir).toContain('quick');
    });

    test('increments task number when existing dirs present', async () => {
  ...
  describe('initVerifyWork', () => {
    ...
    test('has_verification is false with no VERIFICATION.md', async () => {
      var result = initVerifyWork('3');
      expect(result.has_verification).toBe(false);
      expect(result.prior_attempts).toBe(0);
    });

    test('has_verification is true when VERIFICATION.md exists', async () => {
    ...
    test('initVerifyWork respects overrideModel when provided', async () => {
      var result = initVerifyWork('3', path.join(tmpDir, '.planning'), 'inherit');
      expect(result.verifier_model).toBe('inherit');
    });

    test('initVerifyWork falls back to config model when overrideModel is undefined', async () => {
  ...
  describe('initProgress', () => {
    ...
    test('current_phase matches STATE.md', async () => {
      var result = initProgress();
      expect(result.current_phase).toBe(3);
    });

    test('phases is an array', async () => {
      var result = initProgress();
  ...
  describe('statePatch', () => {
    test('updates status field', async () => {
      var result = await statePatch(JSON.stringify({ status: 'building' }));
    ...
    test('updates multiple fields', async () => {
      var result = await statePatch(JSON.stringify({ status: 'built', progress_percent: 80 }));
      expect(result.success).toBe(true);
      expect(result.updated).toContain('status');
    ...
    test('returns error for unknown field', async () => {
      var result = await statePatch(JSON.stringify({ nonexistent_field: 'value' }));
    ...
    test('returns error for invalid JSON', async () => {
      var result = await statePatch('not-json');
      expect(result.success).toBe(false);
    ...
    test('returns error when STATE.md missing', async () => {
      ...
      var result = await statePatch(JSON.stringify({ status: 'built' }));
      expect(result.success).toBe(false);
  ...
  describe('stateAdvancePlan', () => {
    test('returns error when STATE.md missing', async () => {
      fs.rmSync(path.join(tmpDir, '.planning', 'STATE.md'));
      var result = await stateAdvancePlan();
      expect(result.success).toBe(false);
      expect(result.error).toMatch(/STATE.md not found/);
    });

    test('returns error when no Plan: N of M line found', async () => {
    ...
    test('advances plan counter in legacy STATE.md', async () => {
  ...
  describe('stateRecordMetric', () => {
    test('records duration metric', async () => {
      var result = await stateRecordMetric(['--duration', '45m']);
      expect(result.success).toBe(true);
      expect(result.duration_minutes).toBe(45);
    });

    test('records plans_completed metric', async () => {
      var result = await stateRecordMetric(['--plans-completed', '3']);
      expect(result.success).toBe(true);
      expect(result.plans_completed).toBe(3);
    });

    test('records both duration and plans_completed', async () => {
      var result = await stateRecordMetric(['--duration', '30m', '--plans-completed', '2']);
      expect(result.success).toBe(true);
      expect(result.duration_minutes).toBe(30);
      expect(result.plans_completed).toBe(2);
    });

    test('handles hour duration unit', async () => {
      var result = await stateRecordMetric(['--duration', '2h']);
      expect(result.success).toBe(true);
      expect(result.duration_minutes).toBe(120);
    });

    test('succeeds with no args (no-op metric)', async () => {
      var result = await stateRecordMetric([]);
      expect(result.success).toBe(true);
  ...
  describe('detectDrift', () => {
    test('returns drift_detected: false when STATE.md matches filesystem', async () => {
    ...
    test('returns drift_detected: true when STATE.md plans_complete differs', async () => {
    ...
    test('initExecutePhase includes drift field', async () => {
      var result = initExecutePhase('3');
    ...
    test('initPlanPhase includes drift field', async () => {
      var result = initPlanPhase('3');
      expect(result).toHaveProperty('drift');
      expect(typeof result.drift.drift_detected).toBe('boolean');
    });

    test('initProgress includes drift field', async () => {
      var result = initProgress();
      expect(result).toHaveProperty('drift');
      expect(typeof result.drift.drift_detected).toBe('boolean');
    });
  ...
  describe('initResume auto-repair', () => {
    test('returns rederived: true with corrections when state is drifted', async () => {
    ...
    test('returns rederived: false when state is clean', async () => {
