...
describe('module exports', () => {
  test('exports DIMENSIONS as a non-empty array', () => {
    expect(Array.isArray(DIMENSIONS)).toBe(true);
    expect(DIMENSIONS.length).toBeGreaterThan(0);
  });

  test('exports CATEGORIES as an object with expected keys', () => {
  ...
  test('exports PRESETS with expected preset names', () => {
...
describe('dimension schema', () => {
  ...
  test('dimension id matches code:slug pattern', () => {
    for (const d of DIMENSIONS) {
  ...
  test('no duplicate dimension codes', () => {
    const codes = DIMENSIONS.map((d) => d.code);
    expect(new Set(codes).size).toBe(codes.length);
  });

  test('no duplicate dimension ids', () => {
    const ids = DIMENSIONS.map((d) => d.id);
    expect(new Set(ids).size).toBe(ids.length);
  });

  test('categoryCode matches the prefix of code', () => {
    for (const d of DIMENSIONS) {
...
// ---------------------------------------------------------------------------
describe('CATEGORIES', () => {
  test('every dimension category exists in CATEGORIES values', () => {
  ...
  test('every dimension categoryCode exists in CATEGORIES keys', () => {
...
// ---------------------------------------------------------------------------
describe('PRESETS', () => {
  test('minimal preset contains a subset of standard', () => {
    for (const cat of PRESETS.minimal) {
      expect(PRESETS.standard).toContain(cat);
    }
  });

  test('standard preset contains a subset of comprehensive', () => {
    for (const cat of PRESETS.standard) {
      expect(PRESETS.comprehensive).toContain(cat);
    }
  });

  test('comprehensive preset covers all category values', () => {
  ...
  test('custom preset is an empty array', () => {
...
describe('getDimensionById', () => {
  test('finds dimension by full id', () => {
  ...
  test('finds dimension by short code', () => {
...
describe('getDimensionsByCategory', () => {
  test('returns dimensions for self_integrity', () => {
  ...
  test('returns dimensions for audit_config', () => {
...
describe('getActivePresetCategories', () => {
  ...
  test('returns empty array for custom preset', () => {
...
describe('resolveDimensions', () => {
  test('defaults to standard preset with no config or flags', () => {
  ...
  test('results are sorted by code', () => {
    const dims = resolveDimensions({}, {});
  ...
  test('respects config.audit.preset', () => {
  ...
  test('cli preset overrides config preset', () => {
    const config = { audit: { preset: 'comprehensive' } };
  ...
  test('category toggle disables a category', () => {
    const config = {
      audit: {
        preset: 'standard',
  ...
  test('category toggle enables an extra category', () => {
    const config = {
      audit: {
        preset: 'minimal',
  ...
  test('override disables a specific dimension', () => {
    const config = {
      audit: {
  ...
  test('override enables a dimension from a disabled category', () => {
    const config = {
      audit: {
        preset: 'minimal',
  ...
  test('--only with unknown dimension ignores it', () => {
    const dims = resolveDimensions({}, { only: ['SI-01', 'FAKE-99'] });
    expect(dims.length).toBe(1);
    expect(dims[0].code).toBe('SI-01');
  });

  test('--only deduplicates dimensions', () => {
  ...
  test('handles null cliFlags gracefully', () => {
    const dims = resolveDimensions({}, null);
    expect(Array.isArray(dims)).toBe(true);
    expect(dims.length).toBeGreaterThan(0);
  });

  test('--skip with slug works', () => {
    const dims = resolveDimensions({}, { preset: 'standard', skip: ['skill-template-refs'] });
    const codes = dims.map((d) => d.code);
    expect(codes).not.toContain('SI-01');
  });

  test('--dimension with slug works', () => {
...
describe('explainResolution', () => {
  ...
  test('category enable is explained', () => {
    const config = {
      audit: {
        preset: 'minimal',
        categories: { behavioral_compliance: true },
  ...
  test('--dimension add is explained', () => {
  ...
  test('override disable is explained', () => {
    const config = {
      audit: {
        preset: 'standard',
        overrides: { 'SI-01': false },
