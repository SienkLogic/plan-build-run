...
describe('phases list command', () => {
  ...
  test('--type plans lists only PLAN.md files', () => {
    ...
    const result = runPbrTools('phases list --type plans', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('--type summaries lists only SUMMARY.md files', () => {
    ...
    const result = runPbrTools('phases list --type summaries', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('--phase filters to specific phase directory', () => {
    ...
    const result = runPbrTools('phases list --type plans --phase 01', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
...
`
    ...
    const result = runPbrTools('phase-plan-index 04', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
...
`
    ...
    const result = runPbrTools('phase add User Dashboard', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('handles empty roadmap', () => {
    ...
    const result = runPbrTools('phase add Initial Setup', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
...
`
    ...
    const result = runPbrTools('phase insert 1 Fix Critical Bug', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
...
`
    ...
    const result = runPbrTools('phase insert 1 Another Fix', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('rejects missing phase', () => {
    ...
    const result = runPbrTools('phase insert 99 Fix Something', tmpDir);
...
`
    ...
    const result = runPbrTools('phase insert 9.05 Padding Test', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
...
`
    ...
    const result = runPbrTools('phase insert 5 Hotfix', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
...
`
    ...
    // Remove phase 2
    const result = runPbrTools('phase remove 2', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('rejects removal of phase with summaries unless --force', () => {
    ...
    const result = runPbrTools('phase remove 1', tmpDir);
    ...
    const forceResult = runPbrTools('phase remove 1 --force', tmpDir);
  ...
  test('removes decimal phase and renumbers siblings', () => {
    ...
    const result = runPbrTools('phase remove 6.2', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('updates STATE.md phase count', () => {
    ...
    runPbrTools('phase remove 2', tmpDir);
...
`
    ...
    const result = runPbrTools('phase complete 1', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
...
`
    ...
    const result = runPbrTools('phase complete 03.2.1', tmpDir);
...
describe('phase complete milestone-scoped next-phase', () => {
  ...
  test('finds next phase within milestone, ignoring prior milestone dirs', () => {
    ...
    const result = runPbrTools('phase complete 5', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
