...
describe('init commands', () => {
  ...
  test('init execute-phase returns file paths', () => {
    ...
    const result = runPbrTools('init execute-phase 03', tmpDir);
  ...
  test('init plan-phase returns file paths', () => {
    ...
    const result = runPbrTools('init plan-phase 03', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('init phase-op returns core and optional phase file paths', () => {
    ...
    const result = runPbrTools('init phase-op 03', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('init execute-phase extracts phase_req_ids from ROADMAP', () => {
    ...
    const result = runPbrTools('init execute-phase 3', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
...
describe('cmdInitTodos', () => {
  ...
  test('area filter returns only matching todos', () => {
    ...
    const result = runPbrTools('init todos backend', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('area filter miss returns zero count', () => {
    ...
    const result = runPbrTools('init todos nonexistent', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
...
describe('cmdInitPhaseOp fallback', () => {
  ...
  test('normal path with existing directory', () => {
    ...
    const result = runPbrTools('init phase-op 3', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('fallback to ROADMAP when no directory exists', () => {
    ...
    const result = runPbrTools('init phase-op 5', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('neither directory nor roadmap entry returns not found', () => {
    ...
    const result = runPbrTools('init phase-op 99', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
...
describe('cmdInitQuick', () => {
  ...
  test('next number increments from existing entries', () => {
    ...
    const result = runPbrTools('init quick "New task"', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
...
describe('cmdInitNewMilestone', () => {
  ...
  test('file existence flags reflect actual state', () => {
    ...
    const result2 = runPbrTools('init new-milestone', tmpDir);
...
describe('detectDrift', () => {
  ...
  test('returns drift_detected: false when STATE.md matches filesystem', () => {
    // Create STATE.md with v2 frontmatter showing 0 plans complete
    fs.writeFileSync(
      path.join(tmpDir, '.planning', 'STATE.md'),
      '---\nversion: 2\ncurrent_phase: 1\nplans_complete: 0\nplans_total: 1\nprogress_percent: 0\nstatus: "planning"\n---\n\nPhase: 1 of 1\nPlan: 0 of 1\nProgress: [░░░░░░░░░░░░░░░░░░░░] 0%\n'
    );
    // Create a phase with 1 plan and no summary (0 complete)
    const phaseDir = path.join(tmpDir, '.planning', 'phases', '01-setup');
    fs.mkdirSync(phaseDir, { recursive: true });
    fs.writeFileSync(path.join(phaseDir, 'PLAN-01.md'), '# Plan');

    const { detectDrift } = require('../plan-build-run/bin/lib/init.cjs');
    const result = detectDrift(path.join(tmpDir, '.planning'));
    assert.strictEqual(result.drift_detected, false);
    assert.deepStrictEqual(result.stale_fields, []);
  });

  test('returns drift_detected: true when STATE.md plans_complete differs from filesystem', () => {
    ...
    fs.writeFileSync(path.join(phaseDir, 'PLAN-01.md'), '# Plan');
    fs.writeFileSync(
      path.join(phaseDir, 'SUMMARY-01-01.md'),
      '---\nstatus: complete\n---\n# Summary'
    );
    ...
    assert.ok(result.stale_fields.includes('plans_complete'));
  });

  test('initExecutePhase includes drift field in output', () => {
    fs.writeFileSync(
      path.join(tmpDir, '.planning', 'STATE.md'),
      '---\nversion: 2\ncurrent_phase: 1\nplans_complete: 0\nplans_total: 1\nprogress_percent: 0\nstatus: "executing"\n---\n\nPhase: 1 of 1\nPlan: 0 of 1\nProgress: [░░░░░░░░░░░░░░░░░░░░] 0%\n'
    );
    ...
    const result = runPbrTools('init execute-phase 01', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
    const output = JSON.parse(result.output);
    assert.ok('drift' in output, 'execute-phase output should include drift field');
    assert.strictEqual(typeof output.drift.drift_detected, 'boolean');
  });

  test('initProgress includes drift field in output', () => {
    fs.writeFileSync(
      path.join(tmpDir, '.planning', 'STATE.md'),
      '---\nversion: 2\ncurrent_phase: 1\nplans_complete: 0\nplans_total: 0\nprogress_percent: 0\nstatus: "planning"\n---\n\nPhase: 1 of 1\nPlan: 0 of 0\nProgress: [░░░░░░░░░░░░░░░░░░░░] 0%\n'
    );
    ...
    assert.ok(result.success, `Command failed: ${result.error}`);
    const output = JSON.parse(result.output);
    assert.ok('drift' in output, 'progress output should include drift field');
    assert.strictEqual(typeof output.drift.drift_detected, 'boolean');
  });
});
