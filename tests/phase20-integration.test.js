...
describe('INTG-01: install.js PBR branding (no gsd path refs)', () => {
  ...
  test('install.js contains no single-quoted gsd path references', async () => {
    // Match 'gsd' as a string literal used as a directory component
    const matches = installContent.match(/'gsd'/g);
    expect(matches).toBeNull();
  });

  test('install.js references plugins/pbr/commands directory', async () => {
  ...
  test('install.js has PBR branding in banner text', async () => {
...
describe('INTG-02: install.js supports multi-runtime flags', () => {
  ...
  test('install.js supports --claude flag', async () => {
    expect(installContent).toContain('--claude');
  });

  test('install.js supports --opencode flag', async () => {
    expect(installContent).toContain('--opencode');
  });

  test('install.js supports --gemini flag', async () => {
    expect(installContent).toContain('--gemini');
  });

  test('install.js supports --codex flag', async () => {
    expect(installContent).toContain('--codex');
  });

  test('install.js supports --all flag for all runtimes', async () => {
...
describe('INTG-03: package.json publishable structure', () => {
  ...
  test('package.json name is PBR branded', async () => {
...
describe('INTG-04: install.js integrity and test-mode exports', () => {
  test('install.js passes syntax check', async () => {
    expect(() => {
      execSync(`node -c "${INSTALL_JS}"`, { encoding: 'utf8' });
    }).not.toThrow();
  });

  test('install.js loads and exports functions in PBR_TEST_MODE', async () => {
  ...
  test('install.js test-mode exports include PBR-specific functions', async () => {
  ...
  test('directory structure has plugins', async () => {
...
describe('INTG-06: CLAUDE.md architecture documentation', () => {
  ...
  test('CLAUDE.md documents testing commands', async () => {
