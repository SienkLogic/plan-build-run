...
describe('validate-task.js', () => {
  ...
  describe('non-pbr subagent_type', () => {
    ...
    beforeAll(() => {
      // checkNonPbrAgent requires .planning/ to exist in cwd to enforce blocking
      pbrCwd = fs.mkdtempSync(path.join(os.tmpdir(), 'pbr-validate-task-pbr-'));
      fs.mkdirSync(path.join(pbrCwd, '.planning'), { recursive: true });
    });
    ...
    test('non-pbr subagent_type is blocked by enforce-pbr-workflow gate', () => {
      ...
      expect(output).toContain('PBR workflow violation');
...
`;
  ...
  describe('checkpoint manifest advisory', () => {
    ...
    const { checkCheckpointManifest } = require('../hooks/validate-task');
  ...
  describe('active-skill integrity', () => {
    ...
    const { checkActiveSkillIntegrity } = require('../hooks/validate-task');
  ...
  describe('checkDebuggerAdvisory', () => {
    ...
    const { checkDebuggerAdvisory } = require('../hooks/validate-task');
