...
describe('milestone complete command', () => {
  ...
  test('archives roadmap, requirements, creates MILESTONES.md', () => {
    ...
    const result = runPbrTools('milestone complete v1.0 --name MVP Foundation', tmpDir);
  ...
  test('three sequential completions maintain reverse-chronological order', async () => {
    ...
    let result = runPbrTools('milestone complete v1.1 --name Second', tmpDir);
    ...
    result = runPbrTools('milestone complete v1.2 --name Third', tmpDir);
  ...
  test('archives phase directories with --archive-phases flag', async () => {
    ...
    const result = runPbrTools('milestone complete v1.0 --name MVP --archive-phases', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('STATE.md gets updated during milestone complete', async () => {
    ...
    const result = runPbrTools('milestone complete v1.0 --name Test', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('handles missing ROADMAP.md gracefully', async () => {
    ...
    const result = runPbrTools('milestone complete v1.0 --name NoRoadmap', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('scopes stats to current milestone phases only', async () => {
    ...
    const result = runPbrTools('milestone complete v1.1 --name "Second Release"', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('archive-phases only archives current milestone phases', async () => {
    ...
    const result = runPbrTools('milestone complete v1.1 --name Test --archive-phases', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('large phase numbers (456, 457) scope correctly', () => {
    ...
    const result = runPbrTools('milestone complete v1.49 --name DACP', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('handles empty phases directory', async () => {
    ...
    const result = runPbrTools('milestone complete v1.0 --name EmptyPhases', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
...
`;
  ...
  test('handles mixed prefixes in single call (TEST-XX, REG-XX, INFRA-XX)', () => {
    ...
    const result = runPbrTools('requirements mark-complete TEST-01,REG-01,INFRA-01', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('idempotent — re-marking already-complete requirement does not corrupt', async () => {
    ...
    const result = runPbrTools('requirements mark-complete TEST-03', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
