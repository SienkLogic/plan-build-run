...
describe('shouldAutoVerify', () => {
  test('returns false when config.json is missing', async () => {
  ...
  test('returns false when goal_verification is false', async () => {
    fs.writeFileSync(path.join(planningDir, 'config.json'),
      JSON.stringify({ features: { goal_verification: false } }));
    expect(shouldAutoVerify(planningDir)).toBe(false);
  });

  test('returns false for quick depth', async () => {
    fs.writeFileSync(path.join(planningDir, 'config.json'),
  ...
  test('returns true for standard depth', async () => {
    fs.writeFileSync(path.join(planningDir, 'config.json'),
...
describe('getPhaseFromState', () => {
  ...
  test('returns null when no Phase line present', async () => {
    fs.writeFileSync(path.join(planningDir, 'STATE.md'), 'No phase here');
    expect(getPhaseFromState(planningDir)).toBeNull();
  });

  test('parses valid phase info', async () => {
  ...
  test('handles bold Phase Status format', async () => {
    fs.writeFileSync(path.join(planningDir, 'STATE.md'),
...
describe('writeAutoVerifySignal', () => {
  test('creates signal file with phase and timestamp', async () => {
...
describe('isTrustTrackingEnabled', () => {
  test('returns true when config is missing (default)', async () => {
    expect(isTrustTrackingEnabled(planningDir)).toBe(true);
  });

  test('returns true when trust_tracking is not set', async () => {
    fs.writeFileSync(path.join(planningDir, 'config.json'),
      JSON.stringify({ features: {} }));
    expect(isTrustTrackingEnabled(planningDir)).toBe(true);
  });

  test('returns true when trust_tracking is true', async () => {
    fs.writeFileSync(path.join(planningDir, 'config.json'),
      JSON.stringify({ features: { trust_tracking: true } }));
    expect(isTrustTrackingEnabled(planningDir)).toBe(true);
  });

  test('returns false when trust_tracking is false', async () => {
    fs.writeFileSync(path.join(planningDir, 'config.json'),
      JSON.stringify({ features: { trust_tracking: false } }));
    expect(isTrustTrackingEnabled(planningDir)).toBe(false);
  });

  test('returns true when config is invalid JSON', async () => {
