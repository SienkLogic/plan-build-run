...
describe('track-context-budget.js handleHttp parity', () => {
  ...
  test('HTTP returns null when no file_path (parity: CLI returns empty string)', () => {
    const cliOut = runCLI({});
  ...
  test('HTTP returns null when planningDir missing', () => {
  ...
  test('HTTP returns null for small read below all thresholds', () => {
  ...
  test('HTTP updates tracker file on first read', () => {
  ...
  test('HTTP accumulates reads across multiple calls', () => {
  ...
  test('HTTP and CLI both warn on unique-file milestone (10 files)', () => {
    const trackerPath = path.join(planningDir, '.context-tracker');
  ...
  test('HTTP and CLI both warn on char milestone (50k chars)', () => {
    const trackerPath = path.join(planningDir, '.context-tracker');
  ...
  test('HTTP and CLI both warn for large single-file read (>5k chars)', () => {
    ...
    expect(cliParsed.additionalContext).toContain('large file read');
    expect(cliParsed.additionalContext).toContain('huge.js');

    expect(httpOut).not.toBeNull();
    expect(httpOut.additionalContext).toContain('large file read');
    expect(httpOut.additionalContext).toContain('huge.js');
  });

  test('HTTP and CLI both produce no warning past milestone without crossing', () => {
    const trackerPath = path.join(planningDir, '.context-tracker');
  ...
  test('HTTP resets tracker on skill change (same as CLI)', () => {
  ...
  describe('processEvent', () => {
    ...
    test('returns null for plugin root files when pluginRoot set', () => {
    ...
    test('tracks files outside pluginRoot', () => {
      const pluginDir = path.join(tmpDir, 'my-plugin');
      const largeContent = 'x'.repeat(6000);
      const result = processEvent(
      ...
      expect(result.additionalContext).toContain('large file read');
    });
  });
});
