...
const { atomicWrite, validateStatusTransition } = require('../plugins/pbr/scripts/lib/core');
const { sessionLoad, sessionSave } = require('../plugins/pbr/scripts/lib/session');
...
Status: building`;
      ...
      expect(result.last_command).toBe('/pbr:execute-phase 3');
...
next_top_level: something`;
  ...
  describe('resolveDepthProfile', () => {
    ...
    test('returns quick profile with reduced spawns', async () => {
      ...
      expect(result.profile['features.plan_checking']).toBe(true);
      expect(result.profile['features.goal_verification']).toBe('advisory');
...
describe('session subcommand (pbr-tools)', () => {
  ...
  describe('tmux detect', () => {
    test('returns in_tmux false when TMUX not set', async () => {
      ...
      expect(parsed.in_tmux).toBe(false);
      expect(parsed.pane).toBeNull();
    ...
    test('returns in_tmux true with session and pane when TMUX set', async () => {
      const stdout = execFileSync(process.execPath, [SCRIPT, 'tmux', 'detect'], {
        ...
        env: Object.assign({}, process.env, {
          PBR_PROJECT_ROOT: tmpDir,
          TMUX: '/tmp/tmux-1000/default,12345,0',
          TMUX_PANE: '%1',
        }),
      });
      const parsed = JSON.parse(stdout);
      expect(parsed.in_tmux).toBe(true);
      expect(parsed.pane).toBe('%1');
    ...
    test('handles TMUX env with unusual format gracefully', async () => {
      ...
      const parsed = JSON.parse(stdout);
      expect(parsed.in_tmux).toBe(true);
      expect(parsed.session).toBe('simple');
      expect(parsed.pane).toBeNull();
    });
  });
