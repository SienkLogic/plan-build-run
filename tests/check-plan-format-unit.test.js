...
describe('checkStateWrite — syncStateBody branch', () => {
  ...
  test('returns null when body matches frontmatter (all fields)', () => {
    const content = '---\ncurrent_phase: 2\nstatus: "building"\n---\nPhase: 2 of 5\nStatus: Building';
    expect(syncStateBody(content, '/tmp/STATE.md')).toBeNull();
  });

  test('detects and fixes body phase and status drift', () => {
    const filePath = path.join(tmpDir, 'STATE.md');
    const content = '---\ncurrent_phase: 3\nphase_name: "auth"\nstatus: "building"\n---\nPhase: 2 of 5\nStatus: Planning';
  ...
  test('fixes status-only drift (no phase drift)', () => {
    const filePath = path.join(tmpDir, 'STATE.md');
    const content = '---\ncurrent_phase: 2\nstatus: "verified"\n---\nPhase: 2 of 5\nStatus: Building';
    fs.writeFileSync(filePath, content);
    const result = syncStateBody(content, filePath);
  ...
  test('fixes progress_percent drift', () => {
    const filePath = path.join(tmpDir, 'STATE.md');
    const content = '---\ncurrent_phase: 2\nstatus: "building"\nprogress_percent: 60\n---\nPhase: 2 of 5\nStatus: Building\nProgress: [████░░░░░░░░░░░░░░░░] 20%';
    fs.writeFileSync(filePath, content);
    const result = syncStateBody(content, filePath);
...
<task type="auto">
  ...
  test('missing implements: field produces blocking error', () => {
    const content = `---
phase: 03-auth
plan: 01
wave: 1
...
<task type="auto">
  ...
  test('implements:[] present produces no implements error', () => {
  ...
  test('feature task missing behavior element produces blocking error', () => {
  ...
  test('feature task missing implementation element produces blocking error', () => {
    const taskContent = `<task type="auto">
  <name>Task 1</name>
  <files>src/feature.ts</files>
  <action>Do something</action>
  ...
  test('valid feature task with behavior and implementation passes', () => {
    const taskContent = `<task type="auto">
  <name>Task 1</name>
  <files>src/feature.ts</files>
  <action>Do something</action>
...
<task type="auto">
  ...
  test('plan with read_first and acceptance_criteria passes validation', () => {
    const taskContent = `<task type="auto">
  <name>Task 1</name>
  <read_first>
    src/auth/types.ts
  ...
  test('plan missing read_first produces error', () => {
    const taskContent = `<task type="auto">
  <name>Task 1</name>
  <files>src/file.ts</files>
  <action>Do something</action>
  ...
  test('plan missing acceptance_criteria produces error', () => {
    const taskContent = `<task type="auto">
  <name>Task 1</name>
  <read_first>
    src/auth/types.ts
  ...
  test('plan with glob in read_first produces warning', () => {
    const taskContent = `<task type="auto">
  <name>Task 1</name>
  <read_first>
    src/**/*.ts
...
describe('validateContext', () => {
  ...
  test('legacy markdown-header format produces no errors', () => {
    const content = [
      '---',
      'phase: "03-auth"',
      '---',
  ...
  test('completely empty content produces error', () => {
