/**
 * PBR Tools Tests - frontmatter CLI integration
 *
 * Integration tests for the 4 frontmatter subcommands (get, set, merge, validate)
 * exercised through pbr-tools.cjs via execSync.
 *
...
const os = require('os');
const { runPbrTools } = require('./helpers.cjs');

// Track temp files for cleanup
let tempFiles = [];

function writeTempFile(content) {
  const tmpFile = path.join(os.tmpdir(), `pbr-fm-test-${Date.now()}-${Math.random().toString(36).slice(2)}.md`);
...
describe('frontmatter set', () => {
  test('updates existing field', () => {
    ...
    const { extractFrontmatter } = require('../plan-build-run/bin/lib/frontmatter.cjs');
...
body`;
    const file = writeTempFile(content);
    const result = runPbrTools(`frontmatter validate ${file} --schema summary`);
    assert.ok(result.success, `Command failed: ${result.error}`);
    const parsed = JSON.parse(result.output);
...
body`;
    const file = writeTempFile(content);
    const result = runPbrTools(`frontmatter validate ${file} --schema verification`);
    assert.ok(result.success, `Command failed: ${result.error}`);
    const parsed = JSON.parse(result.output);
