...
`
    ...
    const outsideDir = fs.mkdtempSync(path.join(require('os').tmpdir(), 'pbr-test-outside-'));

    try {
      const result = runPbrTools(`state-snapshot --cwd "${tmpDir}"`, outsideDir);
      assert.ok(result.success, `Command failed: ${result.error}`);
...
`
    ...
    const result = runPbrTools(['state', 'add-blocker', '--text', 'Waiting on vendor quote $1.00 before approval'], tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
...
`
    ...
    const result = runPbrTools(`state add-blocker --text-file "${blockerPath}"`, tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
...
`
    ...
    const output = JSON.parse(result.output);
    assert.strictEqual(output.pbr_state_version, '1.0', 'should have version 1.0');
...
`
    ...
    const output = JSON.parse(result.output);
    assert.strictEqual(output.pbr_state_version, '1.0', 'version from frontmatter');
...
`
    ...
    const result = runPbrTools('state update Status "Executing Plan 1"', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);

    const content = fs.readFileSync(path.join(tmpDir, '.planning', 'STATE.md'), 'utf-8');
    assert.ok(content.startsWith('---\n'), 'should start with frontmatter delimiter');
    assert.ok(content.includes('pbr_state_version: 1.0'), 'should have version field');
...
`
    ...
    const result = runPbrTools('state patch --Status "In progress" --"Current Plan" 04-02', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
...
`
    ...
    runPbrTools('state update Status "In progress"', tmpDir);
    runPbrTools('state update Status "Paused"', tmpDir);
...
`
    ...
    runPbrTools('state update Status "Executing Plan 5"', tmpDir);
...
describe('cmdStateLoad (state load)', () => {
  ...
  test('returns raw key=value format with --raw flag', () => {
    ...
    const result = runPbrTools('state load --raw', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
...
describe('cmdStateGet (state get)', () => {
  ...
  test('returns full content when no section specified', () => {
    ...
    const result = runPbrTools('state get', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('extracts markdown section content', () => {
    ...
    const result = runPbrTools('state get Blockers', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('returns error for nonexistent field', () => {
    ...
    const result = runPbrTools('state get Missing', tmpDir);
...
describe('cmdStateRecordMetric (state record-metric)', () => {
  ...
  test('replaces None yet placeholder with first metric', () => {
    ...
    const result = runPbrTools('state record-metric --phase 1 --plan 1 --duration 2min --tasks 1 --files 2', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
...
describe('cmdStateResolveBlocker (state resolve-blocker)', () => {
  ...
  test('adds None placeholder when last blocker resolved', () => {
    ...
    const result = runPbrTools('state resolve-blocker --text "single blocker"', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
...
describe('milestone-scoped phase counting in frontmatter', () => {
  ...
  test('total_phases counts only current milestone phases', () => {
    ...
    const result = runPbrTools('state update Status "Executing"', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);

    // Read the state json to check frontmatter
    const jsonResult = runPbrTools('state json', tmpDir);
  ...
  test('without ROADMAP counts all phases (pass-all filter)', () => {
    ...
    const result = runPbrTools('state update Status "In progress"', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
