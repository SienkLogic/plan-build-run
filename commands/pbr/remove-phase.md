...
```bash
node ${CLAUDE_PLUGIN_ROOT}/scripts/pbr-tools.cjs phase-info $ARGUMENTS
```
...
```bash
node ${CLAUDE_PLUGIN_ROOT}/scripts/pbr-tools.cjs phase remove $ARGUMENTS
node ${CLAUDE_PLUGIN_ROOT}/scripts/pbr-tools.cjs roadmap remove-phase $ARGUMENTS
```

## 5. Update State

```bash
node ${CLAUDE_PLUGIN_ROOT}/scripts/pbr-tools.cjs state record-activity "Removed phase {N}: {name}"
```
