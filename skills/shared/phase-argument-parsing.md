...
3. Phase must be in the expected state for the operation:
   - `/dev:plan N` — phase must not already have plans (unless re-planning)
   - `/dev:build N` — phase must have plans
   - `/dev:review N` — phase must have been built (SUMMARY.md files exist)
...
- No argument and no current phase: "Which phase? Specify a number or run /dev:status to see where you are."
- Phase not found: "Phase {N} not found in .planning/phases/. Run /dev:status to see available phases."
- Wrong state: "Phase {N} hasn't been planned yet. Run /dev:plan {N} first."
